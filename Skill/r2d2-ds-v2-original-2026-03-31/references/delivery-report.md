...
DS depth reached: [visual approx / primitive-token / semantic-token / pattern / canonical-component]
...
Source agreement:
- Figma ↔ source token JSON:  [aligned / partial mismatch / conflict — one-line note]
- Output ↔ source token JSON: [aligned / partial mismatch / conflict — one-line note]
- Screenshot ↔ node data:     [aligned / mismatch — one-line note, only include if validated]
