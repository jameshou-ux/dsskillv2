...
| Skill | Role | Required? |
|-------|------|-----------|
| `figma-use` | Figma reads, writes, variable binding, prototype reactions | Required |
...
If `figma-use` is not installed, stop and tell the user to install it. If `figma-generate-design` is not installed, note the limitation and use the inline generation workflow below instead — output quality will be lower.
...
- Relevant Figma file with DS components and variables (for asset discovery)
...
### Step 3 — Inventory available DS assets in Figma
...
Map intent sections to DS assets. For each section, note which tier of the Incomplete DS Execution Protocol applies.
...
- `approximation` — Tier 5 gap; report what was approximated and why
