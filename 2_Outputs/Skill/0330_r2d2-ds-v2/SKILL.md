...
| `audit-design-system` | DS drift inspection — identifies unbound fills, non-canonical components, naming gaps | DS Governance, DS To Design, and Design To Demo when live Figma inspection is part of the run |
...
Before routing or execution, classify the current run by capability:

- live Figma read
- live Figma write
- local file or code edit
- target codebase inspection
- browser or runtime validation

Do not assume Codex, Claude Code, and other agents share the same tools.

Capability rules:

- If the output is a Figma design or in-Figma prototype, live Figma write is required.
- If the input is an existing Figma design, live Figma read is preferred. If it is unavailable, a supplied design-context bundle is the minimum substitute:
  exact screenshot plus node map, metadata, or design-context export.
- If the output is production-facing code, target codebase inspection and edit capability are required.
- If only screenshots, token JSON, or prose exist, cap DS depth claims to the tier actually evidenced. Never claim `canonical-component` without proof.
- When a required capability is missing, either re-route to the correct lower-bar scenario or stop and ask. Do not silently improvise around the missing capability.
...
1. Canonical source token JSON
2. Canonical Figma component or variable definitions on explicitly canonical pages
3. Supplied Figma design-context bundle from the exact node or surface
4. Governance docs, cleanup history, and screenshots
5. Prompt prose alone

If these disagree, resolve the conflict instead of blending them. Higher-precedence evidence wins unless the user explicitly overrides it.
...
ELSE IF the input is a prompt / PRD / intent AND the output is a demo, prototype,
        or standalone sample code
→ Prompt To Demo

ELSE IF the output requires production-facing code inside a target codebase
        or handoff-grade code mapping
→ DS To Code
...
Standalone HTML, CSS, React, or prototype code without repo integration is still a demo route, not DS To Code.
...
If the current agent cannot pause automatically, it must still send the confirmation message and wait for the user's reply before crossing a hard trigger.
...
2. **Declare capabilities.** State which required capabilities are present before execution. Do not assume another agent has the same Figma, repo, or browser access.
3. **Confirm before substantial execution.** Respect all hard triggers.
4. **Require the right inputs.** Source token JSON is required for all demo and code scenarios. Do not proceed if required inputs are missing.
5. **Respect canonical authority.** DS inputs are the constraint, not soft inspiration. Disagreements between layers must be resolved, not blended.
6. **Apply the Incomplete DS Execution Protocol.** Every region must be traceable to a DS tier. Report every Tier 5 approximation.
7. **Separate demo from production.** Code-looking output is still demo work unless it targets a real codebase with production-facing requirements.
8. **Stop instead of inventing.** If a decision requires inventing DS semantics not supported by sources, stop and ask.
9. **Validate and deliver.** After meaningful Figma writes, validate with both metadata and screenshot. After any substantial run, deliver the Skill-Wide Delivery Report.

---

## Required Inputs

- Clear input and output classification
- Canonical source token JSON (required for all demo and code scenarios; strongly recommended elsewhere)
- Relevant Figma file or page only when the route touches Figma or depends on live canonical asset discovery
- If live Figma read is unavailable for a design-derived task: a supplied design-context bundle for the exact surface
- For prompt-driven work: a prompt, PRD, product note, or design brief
- For code scenarios: target codebase or framework context, plus confirmation that the target surface is canonical and stable
