...
Standalone snippets, HTML samples, or React demos without repo integration are not DS To Code. They belong to Prompt To Demo or Design To Demo even if the output is written in code.
...
- [ ] The target codebase or exact implementation surface is available to inspect and edit
- [ ] There is a real validation path in the target codebase (build, test, lint, visual check, or equivalent)
...
- Ability to inspect and edit the target codebase
...
Inspect the Figma surface and target codebase to confirm they meet the readiness gate:
...
If the codebase is unavailable, stop. Do not output repo-free sample code and call it production-facing handoff.
...
If live Figma read is unavailable but the user supplied an exact design-context bundle, state the limitation clearly and do not claim bounded hardening or canonical instance verification that was not actually performed.
...
- Run the target validation path when available. Report exactly what was run and what was not run.
...
- The target codebase is unavailable or cannot be validated
...
- The target codebase was edited and validated directly
...
- The run cannot inspect or validate the real target codebase
